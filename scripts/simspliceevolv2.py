...
def build_arg_parser():
    ...
    parser.add_argument('-k_eic', '--k_eic',  help="multiplicative constant for exon-intron change (eic) rate  (default=25)",  default = 5) 
    parser.add_argument('-k_indel', '--k_indel', help="multiplicative constant for codon indel rate (default = 5)", default = 5)
    parser.add_argument('-k_tc', '--k_tc', help="multiplicative constant for transcript change (default =5)", default = 5)
    ...
    parser.add_argument('-tc_a5', '--alternative_five_prime', default =  0.25, help="relative frequence of alternative five prime in tc (default =0.1)")
    parser.add_argument('-tc_a3', '--alternative_three_prime', default = 0.25, help="relative frequence of alternative three prime in tc (default =0.1)")
    parser.add_argument('-tc_es', '--exon_skipping', default = 0.3, help="relative frequence of exon skipping in tc (default =0.5)")
    parser.add_argument('-tc_me', '--mutually_exclusive', default = 0.2, help="relative frequence of mutually exclusive in tc (default =0.15)")
    parser.add_argument('-tc_ir', '--intron_retention', default = 0.0, help="relative frequence of intron retention in tc (default =0.00)")
    parser.add_argument('-tc_tl', '--transcript_loss', default = 1.0, help="relative frequence of transcript loss in tc (default =0.4)")
            ...
            for exon_subt in list(set_subt_exons_codons):
                ...
                for _ in range(count_exon_subt):
                    ...
                    if new_pos not in already_picked_positions:
                        if '*' not in exon_subt_sequence[new_pos:new_pos + 3]:
...
def simspliceevol(inputs):
    ...
    SRC, ITERATION_NAME, TREE_INPUT, ONE_NT_DIST, TWO_NT_DIST, THREE_NT_DIST, NT_INTRON_DIST, MEAN_EXON_CDS, MEAN_EXON_LEN, MEAN_INTRON_LEN, SD_EXON_CDS, SD_EXON_LEN, SD_INTRON_LEN, K_NB_EXONS, K_INDEL, C_I, C_D, EIC_ED, EIC_EG, EIC_EL, K_EIC, STOP_CODONS, SPLICE_SITES, CODONS_MATRIX, CODONS_LIST, K_TC, TC_RS, TC_A3, TC_A5, TC_ME, TC_ES, TC_IR, TC_TL = inputs
...
[   0.,    0.,    0.,    0.,    0.,    0.,    0.,    0.,    0.,
    ...
    inputs = [SRC, ITERATION_NAME, TREE_INPUT, ONE_NT_DIST, TWO_NT_DIST, THREE_NT_DIST, NT_INTRON_DIST, MEAN_EXON_CDS, MEAN_EXON_LEN, MEAN_INTRON_LEN, SD_EXON_CDS, SD_EXON_LEN, SD_INTRON_LEN, K_NB_EXONS, K_INDEL, C_I, C_D, EIC_ED, EIC_EG, EIC_EL, K_EIC, STOP_CODONS, SPLICE_SITES, CODONS_MATRIX, CODONS_LIST, K_TC, TC_RS, TC_A3, TC_A5, TC_ME, TC_ES, TC_IR, TC_TL]

    #simspliceevol(inputs)
    try:
        simspliceevol(inputs)
        successful_message()
    except:
